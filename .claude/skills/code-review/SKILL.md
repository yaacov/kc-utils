...
2. Run `make vet`, `make lint`, `make test`, and `make test-e2e-container` (do **not** run `make check` — it writes files via `fmt`). Use `make test-container` for Linux-kernel integration tests when reviewing guest disk backend changes. Capture results for the Lint & Vet section.
