...
2. Run `make vet`, `make lint`, `make test`, and `make test-e2e-container` (do **not** run `make check` — it writes files via `fmt`). On non-Linux hosts also run `make test-container` so `//go:build linux` packages are tested (local `make test` skips them). Capture results for the Lint & Vet section.
