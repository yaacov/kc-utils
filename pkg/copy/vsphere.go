...
func ExportVM(ctx context.Context, host, datacenter string, policy v2vtls.Policy, fingerprint, vmName, secretDir string) (*Lease, error) {
	client, err := vsphere.ConnectHost(ctx, host, policy, fingerprint, secretDir)
	if err != nil {
		return nil, err
	}
