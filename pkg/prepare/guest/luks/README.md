...
`ScanKeyFiles` lists all non-directory entries in the given directory, skipping subdirectories and dotdot-prefixed entries (Kubernetes projected-volume metadata). `KeyFilesMap` wraps `ScanKeyFiles` and returns a `map[string]string` where each key is an `"all"` sentinel (or `"all-N"` when multiple files exist), so the decryptor tries every key on every encrypted volume.
