...
// Backend abstracts privileged disk/mount operations so the direct (host
// syscall), guestfs (libguestfs), and qemu (appliance + kc-agent RPC)
// implementations can be swapped by Mode.
//
// All guestfish CLI and privileged host/appliance tool invocations for guest
// disks must live in Backend implementations under pkg/guest/plugins/.
type Backend interface {
	...
	// Sync flushes pending guest writes to disk. It is a no-op for backends
	// that write live (direct host mounts, guestfs appliance RPC); the qemu
	// backend runs sync(8) inside the appliance.
	Sync() error
