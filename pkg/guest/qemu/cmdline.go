...
func applianceDir() string {
	if d := strings.TrimSpace(os.Getenv(guest.EnvApplianceDir)); d != "" {
		return d
	}
...
func resolveArtifacts() (kernel, initrd, assets string, err error) {
	...
	for _, p := range []string{kernel, initrd} {
		if _, statErr := os.Stat(p); statErr != nil {
			return "", "", "", fmt.Errorf("appliance artifact %s: %w (set %s)", p, statErr, guest.EnvApplianceDir)
		}
	}
...
func guestfsNetworkEnabled() bool {
	v := strings.TrimSpace(os.Getenv(guest.EnvGuestfsNetwork))
