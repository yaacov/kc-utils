...
func (c *client) readFile(path string) ([]byte, error) {
	return c.callBlob(protocol.OpReadFile, protocol.PathArgs{Path: path}, nil, nil)
}

func (c *client) writeFile(path string, data []byte, perm os.FileMode) error {
	_, err := c.callBlob(protocol.OpWriteFile, protocol.WriteFileArgs{Path: path, Perm: perm}, data, nil)
	return err
}

func (c *client) download(guestPath string) ([]byte, error) {
	return c.callBlob(protocol.OpDownload, protocol.PathArgs{Path: guestPath}, nil, nil)
}

func (c *client) upload(guestPath string, data []byte) error {
	_, err := c.callBlob(protocol.OpUpload, protocol.UploadArgs{GuestPath: guestPath}, data, nil)
	return err
}
