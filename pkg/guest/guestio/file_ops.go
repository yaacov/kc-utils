...
func FileReadDir(path string) ([]backend.DirEntry, error) {
	if gp, g, ok := guestPathFromHost(path); ok {
		return g.ReadDir(gp)
	}
	...
	for _, e := range entries {
		...
		out = append(out, backend.DirEntry{Name: e.Name(), IsDir: e.IsDir(), Mode: mode})
	}
	return out, nil
}
...
func FileGlob(pattern string) ([]string, error) {
	if g := guest.Active(); g != nil && g.RootPath() != "" {
		...
		if err == nil && (absPat == root || strings.HasPrefix(absPat, root+string(filepath.Separator))) {
			...
			matches, err := g.Glob(guest.NormalizeGuestPath(rel))
			if err != nil {
				return nil, err
			}
