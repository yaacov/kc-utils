...
The fix reads the first disk's total size and selects a heads value from a lookup table keyed by disk size. It then scans the first disk's partitions for NTFS filesystems, verifies the 8-byte "NTFS    " magic at offset 3, and overwrites the 16-bit heads field at byte offset 0x1A with the computed value. Only the first suitable NTFS partition is patched. The operation is skipped entirely when the version handler's `NeedsNTFSHeadsFix` returns false.
