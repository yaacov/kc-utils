...
func Configure(guestRoot string) {
	...
	if err := guest.FileMkdirAll(modprobeDir, 0o755); err != nil {
		slog.Warn("creating modprobe.d failed", "error", err)
	} else if err := guest.FileWrite(modprobePath, []byte(modCfg.String()), 0o644); err != nil {
...
func cleanStaleAliases(modprobeDir string) {
	entries, err := guest.FileReadDir(modprobeDir)
	if err != nil {
		return
	}
	for _, entry := range entries {
		...
		if changed {
			if err := guest.FileWrite(filePath, []byte(cleaned), 0o644); err != nil {
