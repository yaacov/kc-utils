...
**When not used:** Guests on the [`networkd` handler](../network/handlers/networkd/) skip `Apply` in pipeline block 15; static IPs are written as offline systemd `.network` files instead. Guests on the [`default` handler](../network/handlers/default/) use `Apply` and also [`staticip/`](../network/staticip/) in the same block to write a firstboot `nmcli`/`ip` mapping file.
