...
func init() {
	kernel.Scanners.Register("rpm", &Scanner{})
}
...
func (s *Scanner) SelectBest(kernels []types.KernelInfo) *types.KernelInfo {
	for i := range kernels {
		if kernels[i].Path != "" && kernels[i].InitrdPath != "" {
			return &kernels[i]
		}
	}
	if len(kernels) > 0 {
...
func findVmlinuz(bootFiles map[string]bool, version string) string {
	for _, name := range []string{"vmlinuz-" + version, "vmlinux-" + version} {
		if bootFiles[name] {
			return "/boot/" + name
		}
	}
...
func findInitrd(bootFiles map[string]bool, version string) string {
