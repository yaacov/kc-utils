...
| Function | Role |
|----------|------|
| `Detect` | True for vendor `80-ec2.network`, `ID=amzn` with `VERSION_ID=2023` in os-release, or networkd enabled without active NetworkManager. Amazon Linux 2 (`VERSION_ID=2`) is not matched unconditionally — it falls through to the networkd-enabled check |
...
`Detect` is called once per pipeline run after hypervisor cleanup. When true, block 11b runs `InstallKubeVirtNetworking` and block 15 uses `WriteStaticNetworks` instead of `nicnaming` + firstboot. EC2 net-hook masking is handled by [`systemd.DisableEC2NetHooks`](../systemd/) from the EC2 hypervisor plugin.
